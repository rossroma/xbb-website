<template>
  <aside
    class="fixed top-1/2 right-1.5 z-12 -translate-y-1/2 flex flex-col items-center gap-1.5 max-[1199px]:right-0 max-[1199px]:gap-1 max-[780px]:hidden"
    aria-label="快捷操作"
  >
    <!-- 客服入口（顶部大图标） -->
    <a
      ...
      target="_blank"
      rel="noreferrer"
      class="w-17 max-[1199px]:w-12 relative block no-underline hover:opacity-90 transition-opacity duration-fast motion-reduce:transition-none"
    >
      <img :src="topService.icon" :alt="topService.label" class="w-full h-auto block" />
      <span
        class="absolute left-1/2 bottom-1.5 -translate-x-1/2 min-w-14 leading-6 px-2 rounded-pill bg-toolbar-scroll-top-gradient text-white text-caption font-semibold text-center whitespace-nowrap shadow-toolbar-service-label max-[1199px]:min-w-10 max-[1199px]:leading-4 max-[1199px]:px-1.5 max-[1199px]:text-caption max-[1199px]:bottom-1"
    ...
    <!-- 面板 -->
    <div
      class="w-13 bg-white pt-2.5 pb-12 rounded-pill shadow-toolbar relative max-[1199px]:w-9 max-[1199px]:pt-2 max-[1199px]:pb-8"
    >
      <template v-for="item in items" :key="item.key">
        ...
        <a
          ...
          class="group w-full text-center py-2.5 px-0 border-0 bg-transparent cursor-pointer relative block no-underline max-[1199px]:py-1.5"
        >
          <div
        ...
        >
          ...
          <span
            :class="[
              'w-30 h-30 rounded-lg absolute right-6 top-1/2 -mt-17 bg-white shadow-toolbar z-3 transition-all duration-glide ease-in-out motion-reduce:transition-none max-[1199px]:w-24 max-[1199px]:h-24 max-[1199px]:right-2.5 max-[1199px]:-mt-14',
              visibleMap[item.key]
                ? 'opacity-100 right-13 visible max-[1199px]:right-9'
                : 'opacity-0 invisible',
            ]"
            :style="{
        ...
        >
          ...
          <span
            :class="[
              'leading-12 h-12 text-toolbar-icon text-caption px-6 rounded-l-full absolute right-6 top-1/2 -translate-y-1/2 bg-white shadow-toolbar z-0 transition-all duration-glide ease-in-out motion-reduce:transition-none whitespace-nowrap max-[1199px]:leading-9 max-[1199px]:h-9 max-[1199px]:text-caption max-[1199px]:px-5 max-[1199px]:right-2.5 max-[1199px]:-translate-y-1/2',
              visibleMap[item.key]
                ? 'opacity-100 right-13 visible max-[1199px]:right-9'
                : 'opacity-0 invisible',
            ]"
