<template>
  <div class="show-info-management">
    <div class="page-header">
      <h2>展示信息管理</h2>
      <el-button type="primary" @click="showCreateDialog">新增展示信息</el-button>
    </div>
    ...
    <div class="search-area">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <el-form-item>
          <el-button type="primary" @click="loadShowInfos">搜索</el-button>
    ...
    <!-- 展示信息列表 -->
    <el-table :data="showInfos" v-loading="loading" border>
    ...
    >
      ...
      >
        ...
        <el-form-item label="缩略图">
          <el-input v-model="form.simg" placeholder="请输入图片URL" />
        </el-form-item>
...
import { getShowInfos, createShowInfo, updateShowInfo, deleteShowInfo } from '@/shared/api/gallery'

// 响应式数据
const loading = ref(false)
const showInfos = ref<Record<string, unknown>[]>([])
const dialogVisible = ref(false)
const dialogTitle = ref('新增展示信息')
...
const loadShowInfos = async () => {
  ...
  try {
    ...
    const result = await getShowInfos(params) as unknown as { items: Array<{
      id: number
      title: string
      subtitle: string
      bid: number
    ...
    showInfos.value = result.items || []
    pagination.total = result.total || 0
  } catch (error) {
    console.error('加载展示信息失败:', error)
    ElMessage.error('加载展示信息失败')
  } finally {
    loading.value = false
  }
}
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      await updateShowInfo(editId.value, form)
      ElMessage.success('更新成功')
    } else {
      await createShowInfo(form)
